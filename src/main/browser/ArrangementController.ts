...
export class ArrangementController {
   ...
   *
   * ## Why from the broadcast round
   *
   * The arrangement used to be recorded at exactly one point, that collapse, so a split made by
   * dragging a tab into an edge, one chosen from the layout menu and one brought back by session
   * restore had no recording at all. Every one of them settles in the round that already exists
   * for "something about this window changed". Hooking each cause instead would mean finding all
   * of them, and that list has grown twice already. Writing it every time is also what makes the
   * way back safe: a recording that is always current cannot describe a state the user has moved
   * on from, so applying one need not spend it.
   *
   * The round used to hold its broadcast flag down across this call, because the group version
   * published when it changed something. A recording is invisible to the renderer and carries no
   * IPC channel (KTD6), so nothing here publishes and the window needs no re-entrancy guard —
   * carrying one anyway would swallow a genuine request arriving from elsewhere in the round.
   *
   ...
   * means a recording was applied, none means nothing was. That keeps every reason to decline in
