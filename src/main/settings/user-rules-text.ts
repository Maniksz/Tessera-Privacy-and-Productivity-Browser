...
const en = {
   ...
   *
   * The second sentence is the one that matters and is not decoration: a rule from the normal profile keeps
   * hiding its element in a private window whatever is done to it here, because stored rules reach every
   * window through the engine's one global slot and a private window can only *add* to what its pages get.
   * See `SessionUserRuleEditor.applySource`.
   */
  sessionNote:
    'This is a private window. What you change here lasts until the last private window closes and is not saved. Rules from your normal profile keep applying in this window even if you switch them off or delete them here.',
...
const de: UserRulesText = {
  ...
  sessionNote:
    'Das ist ein privates Fenster. Was du hier änderst, gilt, bis das letzte private Fenster geschlossen wird, und wird nicht gespeichert. Regeln aus deinem normalen Profil wirken in diesem Fenster weiter, auch wenn du sie hier ausschaltest oder löschst.',
