...
export const OVERLAY_PRECEDENCE = {
  ...
  'tab-drop': 2,
  /**
   * Ranked with the menu and the drag, which is the one rank that keeps both of its neighbours honest.
   *
   * *Above the find bar and the tile bar*, because this list is the answer to a deliberate press on a
   * badge and both of those can be claimed without one: a pointer drifting towards a tile's top edge
   * must not take down the account picker somebody is reading, or the same accident that is barred from
   * refusing a consent dialogue would instead throw away a fill request — and with it the one-shot
   * consent that press had earned.
   *
   * *Not above a permission, navigation or master-password prompt*, and that is the load-bearing half.
   * The presses that raise this surface come from a page view, over a channel the page's own preload
   * uses; ranking it higher would make "the page's badge" a way to displace a consent dialogue the user
   * was reading, which is precisely the answering-on-the-user's-behalf that this table exists to stop.
   * Below `master-password` for the further reason that this surface's own unlock button *raises* that
   * prompt: a picker that outranked the prompt it summons would take the layer back from it.
   *
 ...
 *
 * The three `tile` surfaces carry their own rectangle rather than being handed a tile index to resolve.
 * All three are computed where the facts are: the tile bar's from the same function that positions the
 * views, the find bar's from the searched view's own bounds, the suggestion list's from a field rectangle
 * the page reported and the zoom and bounds of the view it sits in (`passwords/suggest-bounds.ts`). A
 * second resolution here would be a second opinion about where a tile is, and the two eventually disagree
 * — and for the list there is nothing to have an opinion *with*: this module has never heard of a field.
 */
