...
export const OVERLAY_REGION = {
  ...
  'picker-bar': 'tile',
  /**
   * The whole window, for the layout menu's reason: it hangs from a toolbar button, and a click anywhere
...
export function surfaceIdentity(presentation: OverlayPresentation): string {
  switch (presentation.kind) {
    ...
    */
    ...
    /*
      A singleton, like the menu: one window has one downloads button and one panel. Keyed by kind so that
      every re-presentation with fresh rows is an update — which is what keeps the keyboard where the user
      put it; see `OVERLAY_REFOCUSES_ON_UPDATE`.
    */
...
export const OVERLAY_PRECEDENCE = {
  ...
  'master-password': 4,
  /**
   * Level with the layout menu, and for the menu's reasons: it is opened from a toolbar button on purpose,
   * so a hovered tile bar may not take it down, and it gives way to every dialogue above it — nothing is
   * waiting on a list of downloads.
   *
   * The rank does not decide the one thing that would be dangerous here, which is the panel's own
   * re-presentations. Equal replaces, so a core that re-presented the panel on a progress tick after the
...
/**
 * Whether an *update* — the same surface re-presented — moves the keyboard into the layer again.
 *
 * ## Why every existing kind says yes
 *
 * Because that is what they have always done, and two of them depend on it. A find bar and a picker bar
 * are re-presented as results arrive, and the user may have clicked into the page in between; the next
 * update pulling the keyboard back into the bar is how Escape and Return keep reaching it. Changing that
 * here would change those bars, which is not this table's business.
 *
 * ## Why the downloads panel says no
 *
 * It is re-presented up to four times a second while a download runs (KTD8), not because anybody asked
 * for it again. Focusing the layer's web contents on each of those would at best do nothing and at worst
 * pull the keyboard back from wherever the user has since taken it — and inside the panel the focus
 * belongs to a row and a button the user chose, which the surface keeps by download id across updates. The first presentation still takes the keyboard
 * (`takesFocus`); only the updates leave it where it is.
 */
export const OVERLAY_REFOCUSES_ON_UPDATE = {
  'layout-menu': true,
  'tab-drop': true,
  'permission-request': true,
  'tile-bar': true,
...
/**
 * Whether presenting `incoming` over `outgoing` moves the keyboard into the layer.
 *
 * The one question `OverlayLayer.present` asks about focus, kept here so it can be answered without a
 * window: a surface that takes focus takes it when it arrives, and on an update only if its kind says so.
 */
export function movesFocus(incoming: OverlayPresentation, outgoing: OverlayState): boolean {
