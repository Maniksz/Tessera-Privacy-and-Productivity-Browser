 ...
 *
 * This file drives the seams the way `BrowserWindowController` drives them — `round()` below is that
 * class's `#maintainArrangement`, and `activate()` is the `tile === null` branch of its
...
async function harness(options: {
  ...
  /*
    Never called by anything these tests exercise, and cast rather than built because building one
    means an Electron `WebContentsView`. The seams that reach it — drag, fullscreen, the tile bar —
    are constructed here and asked nothing.
  */
