...
const STATE_KEYS = ['autofill.suggest.locked', 'autofill.suggest.empty', 'autofill.suggest.disabled']
...
describe('what a screen reader is told', () => {
  ...
  it('names the tile the field is in, so two lists are not the same list', () => {
    ...
    expect(screen.getByRole('group', { name: t('autofill.suggest.label', { index: 3 }) })).toBeTruthy()
  })
...
describe('the list does not close itself', () => {
  ...
  it('does not dismiss when a press lands on its own padding', () => {
    const handlers = draw()
    fireEvent.pointerDown(screen.getByRole('group', { name: t('autofill.suggest.label', { index: 2 }) }))
    expect(handlers.dismissed).toEqual([])
  })
})
...
describe('both catalogues carry the wording', () => {
  ...
  it('translates the refusals rather than repeating the English', () => {
    ...
    for (const key of Object.values(REFUSAL_KEYS)) {
      expect(
        (catalogs.de as Record<string, string>)[key],
        `${key} was not translated`
      ).not.toBe((catalogs.en as Record<string, string>)[key])
    }
  })
})
