 ...
 *
 * What is *not* here, named rather than quietly missing: putting a factor on a live view, raising the
 * pinch limits that hand the trackpad to the engine, the subscription that carries a zoom gesture in,
 * and the `did-navigate` re-assert. All four need a window. The gesture's own decisions — what counts
 * as one, and which pane it lands on — are covered in `wheel-zoom.test.ts`. The round trip that
 * carries a pane's zoom across a restart is reachable and is pinned in the session tests.
 */
