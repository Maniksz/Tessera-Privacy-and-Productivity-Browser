...
/**
 * Whether this run reads an instrumented copy of the runtime, as the mutation run does.
 *
 * Stryker rewrites every branch to call its own counters, which live at file scope. Serialised and
 * re-compiled with no scope, that copy throws `ReferenceError` on its first branch, which says nothing
 * about the runtime and stopped the whole mutation run in its dry run. So the compiled harness steps
 * aside there, and the in-place one below — which exists for exactly this reason — carries the run.
 */
const INSTRUMENTED = runScriptlets.toString().includes('stryMutAct')
...
describe('the runtime compiled from its own source', () => {
  it.skipIf(INSTRUMENTED)('compiles from its own source with no scope, and runs', () => {
    /*
...
describe.each(INSTRUMENTED ? [IN_PLACE] : [COMPILED, IN_PLACE])(
  'the runtime, $name',
  ({ page, runnerFor }) => {
    ...
    describe('set-constant', () => {
      ...
      */
      ...
      it('reads a path with no names in it as no path at all', () => {
        const world = page()
        expect(() =>
          runnerFor(world)([{ name: 'set-constant', args: ['.', 'true'] }])
        ).not.toThrow()
