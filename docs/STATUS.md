...
| **`TileFullscreenController` hat einen unerreichbaren Zweig** | **Behoben (24.09.2026).** `SplitController.escape()` gibt die Kachel mit dem Urteil zurück (`EscapeStep`), der Null-Wächter ist weg. Der zweite offene Zweig (`tabId === null`) war erreichbar — ein geschlossener Tab verlässt das Raster, nicht das Kachel-Vollbild — und hat jetzt einen Test. Die Datei hat eine Untergrenze von 100 % in allen vier Maßen |
...
| Nachtrag | Zwei Tabs kacheln, im Layout-Menü „Einzeln“ wählen, dann den anderen Tab anklicken | Er bekommt das ganze Fenster, der Split kommt nicht zurück; beim Kacheln erscheint kein Gruppen-Chip |
| Nachtrag | Eine Gruppe einklappen, Rechtsklick auf ihren Chip → „Gruppierung auflösen“ | Der Chip verschwindet, beide Tabs bleiben offen |
| Nachtrag | Auf einer Seite in den DevTools: `setTimeout(() => location.href = 'https://example.com/?' + 'utm_x=1&'.repeat(200), 2000)` | Die Nachfrage „Diese Seite möchte dich zu example.com schicken“ zeigt die Adresse gekürzt mit „Ganze Adresse zeigen“; ausgeklappt scrollt nur die Adresse, beide Knöpfe bleiben sichtbar |
